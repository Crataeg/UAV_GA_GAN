...
def main():
    ...
    if not bool(args.skip_measured_corner_compare) and datasets_ready:
        measured_out = os.path.join(base_output, "measured_cornercases")
        try:
            ...
            print(f"[WARN] measured corner-case comparison failed (non-fatal): {e}")
    elif not datasets_ready:
        print("[INFO] measured corner-case comparison skipped: dataset zip(s) not found")
    ...
    if not bool(args.skip_measured_corner_compare) and datasets_ready:
        print("Measured corner-case outputs:", os.path.join(base_output, "measured_cornercases"))
    if viz_count > 0:
