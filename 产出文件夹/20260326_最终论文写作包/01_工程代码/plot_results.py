...
def plot_kpi_comparison(report: Mapping, out_dir: str) -> None:
    ...
    # 2) Rate stats
    sinr_stats = {g: _get_stat(report, g, "sinr_db") for g in groups}
    fig, ax = plt.subplots(figsize=(6, 4))
    _bar_with_quantiles(ax, groups, sinr_stats, "SINR (core comparison metric)", "dB")
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, "kpi_sinr_stats.png"), dpi=150)
    plt.close(fig)
